...
public class EstimateServiceTests
{
    ...
    [Fact]
    public async Task ConvertToInvoiceAsync_EmptyTransitionBody_FetchesEstimate()
    {
        Func<HttpRequestMessage, HttpResponseMessage> respond = request =>
        {
            if (request.Method == HttpMethod.Put)
            ...
            Assert.EndsWith("/estimates/9", request.RequestUri!.AbsolutePath, StringComparison.Ordinal);
            return new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new StringContent("""
                {
    ...
    [Fact]
    public async Task GetEstimateAsync_WithHydration_FetchesLinkedResources()
    {
        var calls = new List<string>();
        Func<HttpRequestMessage, HttpResponseMessage> respond = request =>
        {
            calls.Add(request.RequestUri!.AbsolutePath);
            if (request.RequestUri.AbsolutePath.EndsWith("/estimates/42", StringComparison.Ordinal))
            {
                return new HttpResponseMessage(HttpStatusCode.OK)
                {
                    Content = new StringContent("""
        ...
        var estimate = await service.GetEstimateAsync(
            42,
            new EstimateGetOptions { IncludeContact = true, IncludeProject = true });

        Assert.Equal("Example Ltd", estimate.Contact?.OrganisationName);
        Assert.Equal("Example project", estimate.Project?.Name);
    ...
    public async Task ListAutoPagingAsync_YieldsAllPages()
    {
            ...
            var response = new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new StringContent(page == 1
                    ? """
                      {
                        "estimates": [
                          { "url": "https://api.freeagent.com/v2/estimates/1", "reference": "001" },
                          { "url": "https://api.freeagent.com/v2/estimates/2", "reference": "002" }
                        ]
                      }
                      """
                    : """
                      {
                        "estimates": [
                          { "url": "https://api.freeagent.com/v2/estimates/3", "reference": "003" }
                        ]
                      }
                      """)
            };
        ...
        await foreach (var estimate in service.ListAutoPagingAsync(perPage: 2))
        {
            references.Add(estimate.Reference!);
        }
    ...
    [Fact]
    public async Task DuplicateEstimateAsync_PostsDuplicateEndpoint()
    {
        var handler = new QueueHttpMessageHandler(request =>
        {
            Assert.Equal(HttpMethod.Post, request.Method);
            Assert.EndsWith("/estimates/5/duplicate", request.RequestUri!.AbsolutePath, StringComparison.Ordinal);
            return new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new StringContent("""
                {
        ...
        var estimate = await service.DuplicateEstimateAsync(5);

        Assert.Equal(6, estimate.ResourceId);
        Assert.Equal(EstimateStatus.Draft, estimate.Status);
        Assert.Equal("002", estimate.Reference);
    }

    [Fact]
    public async Task UpdateEstimateItemAsync_PutsEstimateItemPayload()
    {
        string? postedJson = null;
        var handler = new QueueHttpMessageHandler(request =>
        {
            Assert.Equal(HttpMethod.Put, request.Method);
            Assert.EndsWith("/estimate_items/4", request.RequestUri!.AbsolutePath, StringComparison.Ordinal);
            postedJson = request.Content!.ReadAsStringAsync().GetAwaiter().GetResult();
            return new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new StringContent("""
                {
                  "estimate_item": {
                    "url": "https://api.freeagent.com/v2/estimate_items/4",
        ...
        var item = await service.UpdateEstimateItemAsync(4, new EstimateItem
        {
            Description = "Updated line",
            ItemType = EstimateItemType.Hours,
            Quantity = 2,
        ...
        Assert.NotNull(postedJson);
        Assert.Contains("\"description\":\"Updated line\"", postedJson, StringComparison.Ordinal);
        Assert.Equal("Updated line", item.Description);
    }

    [Fact]
    public async Task DeleteEstimateItemAsync_DeletesResource()
    {
        var handler = new QueueHttpMessageHandler(request =>
        {
            Assert.Equal(HttpMethod.Delete, request.Method);
        ...
        await service.DeleteEstimateItemAsync(4);
    }

    [Fact]
    public async Task GetEstimatePdfAsync_InvalidBase64_ThrowsFreeAgentApiException()
    {
        var handler = new QueueHttpMessageHandler(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent("""
        ...
        await Assert.ThrowsAsync<FreeAgentApiException>(() => service.GetEstimatePdfAsync(1));
    }
}
