...
| `ShowProjectName` | `bool?` | `show_project_name` | Writable on create and on draft updates; omitted once the invoice leaves draft; requires `Status` on update when set |
