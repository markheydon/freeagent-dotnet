...
Demo data for the FreeAgent.NET SDK sample app and future API probe seeders. Files are **read-only snapshots** copied from the upstream [Turpinverse](https://github.com/markheydon/turpinverse) repository (`canon/` folder). This SDK repository does not modify Turpinverse; it only vendors the JSON files needed to seed FreeAgent API probes.
...
- **Contacts** — one FreeAgent B2B contact per organisation (`tradingName` + primary contact persona name/email/phone; `registeredOffice` for billing address). See `samples/FreeAgent.Client.BlazorSample/Services/Turpinverse/`.
